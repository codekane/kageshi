...
dongCheck.then(dongGot, gotError);


// Stuff to normalize fonts
var fontNormalization = `
  .fs_0 { font-size: 1em !important; }
  .fs_2 { font-size: 1em !important; }
  .fs_3 { font-size: 1em !important; } `

let fontChecked;
function fontGot(item) {
  if (item.normFonts == true) {
    let css = document.createElement("style");
    css.setAttribute("type", "text/css");
    css.setAttribute("rel", "stylesheet");
    css.innerHTML = fontNormalization;
    document.head.appendChild(css);
  }
}
...
var scaldStyles = document.createElement('style');
scaldStyles.type = 'text/css';
scaldStyles.innerHTML = fontNormalization;
document.head.appendChild(scaldStyles);
